<template>
  <v-content style="margin: 10px">
    ...
    <v-container>
      ...
      <v-card>
        <v-col cols="12" v-if="!idTipoConta">
          <v-card-text>
            <p class="display-1 text--primary">Novo Tipo de Conta</p>
            <div>Cadastro de Categorias de Contas</div>
          </v-card-text>
        </v-col>

        <v-col cols="12" v-if="idTipoConta">
          <v-card-text>
            <p class="display-1 text--primary">Editar Tipo de Conta</p>
            <div>Edção de dados de Categorias de Contas</div>
          </v-card-text>
        </v-col>
        ...
        <v-col cols="12" sm="12">
          <v-card-actions>
            ...
            <v-btn
              v-if="!isMobile && idTipoConta"
              color="secondary darken-1"
              large
              dark
              width="150"
              @click="updateTipoConta()"
            ...
            <v-btn
              v-else-if="isMobile && !idTipoConta"
              color="secondary darken-1"
              large
              dark
              width="150"
...
import { TipoContaFormController } from "../../controllers/Cadastro/TipoContaFormController";
...
export default class TipoContaForm
  extends Vue
  implements ITipoContaFormView {
  _controller?: ITipoContaFormController;
  ...
  fecharDialogMessage() {
    ...
    if (this.isSucesso) {
      this.$router.push("/tipoContas");
    }
  }

  retornaListaTipoContas() {
    this.$router.push("/tipoContas");
  }
  ...
  validate() {
    if (
      this.tipoConta.descricao == "" ||
      this.tipoConta.descricao.length > 30
    ) {
      this.colorBorder = "error";
      this.color = "error";
      this.text =
        "O campo Descrição do Tipo de Conta está vazio ou inválido.";
      this.snackbar = true;
      return true;
    }
  ...
  saveTipoConta() {
    ...
    if (this.validate()) {
      ...
      this._controller!.salvar(this.tipoConta);
    }
  }
  ...
  updateTipoConta() {
    if (this.validate()) {
      ...
      //this.solicitacao.user = parseInt(Storage.get("User")!);
      this._controller!.update(this.idTipoConta, this.tipoConta);
    }
  }

  carregarDtos(): void {
    this.idTipoConta = this.$route.params.id;
  }

  showTipoContaForEditar(item: any) {
    this.picker = item.color
    this.tipoConta = item;
  }

  created() {
    this._controller = new TipoContaFormController(this);
