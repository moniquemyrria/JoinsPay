<template>
  <v-content style="margin: 10px">
    ...
    <k-dialog
      :dialog="dialogDelete"
      title="Remover Tipo de Conta?"
      text="A Categoria de Conta selecionada será removida. Deseja Continuar?"
    >
    ...
    <v-container>
      ...
      <v-row>
        <v-col cols="10">
          <v-card-text>
            <p class="display-1 text--primary">Tipos de Conta</p>
            <div>Lista de Categorias de Contas</div>
          </v-card-text>
        </v-col>
        <v-spacer></v-spacer>
      ...
      <v-row>
        <v-col cols="12" sm="12">
          <v-col cols="12" sm="5" style="float: right">
            <v-text-field
              v-model="search"
              label="Pesquisa Tipos de Contas"
              class="mx-4"
...
import {
  ITipoContaListaView,
  ITipoContaListaController,
} from "../../contracts/Cadastro/TipoContaListaContract";

import { TipoContaListaController } from "../../controllers/Cadastro/TipoContaListaController";
...
export default class TipoContaLista extends Vue implements ITipoContaListaView {
  _controller?: ITipoContaListaController;
  ...
  deleteItem(item: any) {
    this.itemTipoContaDelete = item;
    this.dialogDelete = true;
  }

  deleteItemConfirmado() {
    this.dialogDelete = false;
    this._controller!.deleteitem(this.itemTipoContaDelete.id);
  }

  editarItem(item: any) {
    this.$router.push("/tipoConta/" + item.id);
  }
  ...
  addSolicitacao() {
    this.$router.push("/tipoConta");
  }

  created() {
    this.isMobile = Display.isMobile();
    this._controller = new TipoContaListaController(this);
