<template>
  <v-content style="margin: 10px">
    ...
    <k-dialog
      ...
      text="A Categoria de Despesa selecionada será removida. Deseja Continuar?"
    >
    ...
    <v-container>
      ...
      <v-row>
        <v-col cols="10">
          <v-card-text>
            <p class="display-1 text--primary">Tipos de Despesa</p>
            <div>Lista de Categorias de Despesas</div>
          </v-card-text>
        </v-col>
        <v-spacer></v-spacer>
      ...
      <v-row>
        <v-col cols="12" sm="12">
          <v-col cols="12" sm="5" style="float: right">
            <v-text-field
              v-model="search"
              label="Pesquisa Tipos de Despesas"
              class="mx-4"
...
import {
  ITipoDespesaListaView,
  ITipoDespesaListaController,
} from "../../contracts/Cadastro/TipoDespesaListaContract";

import { TipoDespesaListaController } from "../../controllers/Cadastro/TipoDespesaListaController";
...
export default class TipoDespesaLista extends Vue implements ITipoDespesaListaView {
  _controller?: ITipoDespesaListaController;
  ...
  deleteItem(item: any) {
    this.itemTipoDespesaDelete = item;
    this.dialogDelete = true;
  }

  deleteItemConfirmado() {
    this.dialogDelete = false;
    this._controller!.deleteitem(this.itemTipoDespesaDelete.id);
  }

  editarItem(item: any) {
    this.$router.push("/tipoDespesa/" + item.id);
  }
  ...
  addSolicitacao() {
    this.$router.push("/tipoDespesa");
  }

  created() {
    this.isMobile = Display.isMobile();
    this._controller = new TipoDespesaListaController(this);
